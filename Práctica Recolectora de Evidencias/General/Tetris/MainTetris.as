package {
	...
	public class MainTetris extends MovieClip {
		...
		//variable para ir al frame del selector de jugadores
		var nivelBool: Boolean = false;
		var jugador: Array = new Array(); //array para guardar el nombre del jug
		var Gjugador:Boolean=true; //saber si ya se guardo un nombre de jugador
		var Sjugador:Boolean=false; //saber si ya se guardo para procedaer al frame del juego
		//var jug1b:Boolean;

		public function MainTetris() {//funcion donde inicializamos el array de filas
			for (i = 0; i < 15; i++) {
				filas.push(0);
			}
			//funciones para conectarse a otro swf
			ml.connect("multi2");//conectarnos al otro swf con ese nombre
			ml.client=this;
		...
		function perdio() {
			...
			if (perder == true) {//comparamos si perder es igual a true, si es asi significa que se perdio
				...
				nextFrame();
				//cuadro de texto en donde se imprimiran los resultados del jugador
				player1_txt.text="Jugador: "+jugador +", Tiempo: "+ min+" m." + " " +sg+" s."+", Puntos: "+ puntos;
				//texto2.text="Perdi1";
				stage.addEventListener(Event.ENTER_FRAME, Validarjug);
				//ml.send("multi1", "receptor2", player1_txt.text);
		...
		//funcion para hacer aparecer las opciones de reglas
		...
		//funcion para hacer aparecer los botones de los niveles y opciones
		...
		//fucnion para abrir la ventana de niveles
		...
		//boton que se usara para cerrar las reglas o los niveles
		public function Cerrar(event: MouseEvent) {
			CerrarTodo();
		}
		//funcion para hacer desaparecer los objetos en pantalla para las reglas y los niveles
		function CerrarTodo() {
			trans_mc.visible=false;
			reglas_mc.visible=false;
			x_mc.visible=false;
		...
		//funcion para el selector de niveles, en donde solo se cambiara la variablde de velocidad
		...
		//funcion para boton para pasar al juego
		public function Siguiente4(event: MouseEvent) {
			//si esta variable es verdadera se podra acceder al 3er frame
			if (Sjugador==true){
				gotoAndStop(3);
				timer2.start();
				stage.focus=this;
				stage.addEventListener(Event.ENTER_FRAME, Caida);
				rdm = Math.random() * 7 + 1; //variable del random para la caida de las figuras
				siguiente_btn2.addEventListener(MouseEvent.CLICK, Siguiente2);
				timer.addEventListener(TimerEvent.TIMER, tiempo);
		...
		//funcion paraguardar el nombre del jugador en un arreglo, y hacer que se pueda seguir a otro frame
		...
		//funcion para ir a un frame anterior, asignar los event listener y hacer invisibles
		...
		//esta funcion sirve para imprimir en pantalla lo que se envio mediante el otro swf
		public function receptor1 (e1:String){
			//if(jug1b==true){
				player2_txt.text=e1;
			//}else{
			//	player2_txt.text=e11
			//}
		}
		/*public function preceptor1 (event:int){
			if(tjug2 > tjug1){
				jug1b=true;
			}else{
				jug1b=false;
		...
		//Esta funcion servira para que se ejuecute constantemente , y hasta que se agrege al otro swf
		public function Validarjug (event:Event){
			//enviar un dato a otro swf, en donde primero ira nuestro nombre de swf , despues al que queramos conectar, y por ultimo nuestra informacion
			ml.send("multi1", "receptor2", player1_txt.text);
			//ml.send("multi1", "receptor2", tjug1);
		}
	}
